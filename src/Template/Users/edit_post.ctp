<div class="list-addpost-admin">
    ...
    <!-- Form Edit Post -->
    <?= $this->Form->create('', ['id' => 'form-edit-post', 'type' => 'file']);?>
    <div class="col-md-8">
        <div class="form-group">
            <?= $this->Form->input('post_title', ['label' => 'Tên bài viết', 'class' => 'form-control', 'value' => $editPost->post_title]);?>
        </div>
        <div class="form-group">
            <?= $this->Form->input('post_slug', ['label' => 'Đường dẫn', 'class' => 'form-control', 'value' => $editPost->post_slug]);?>
        </div>
        <div class="form-group">
            <?= $this->Form->input('post_description', ['label' => 'Mô tả ngắn', 'class' => 'form-control', 'value' => $editPost->post_description]);?>
        </div>
        <div class="form-group">
            <?= $this->Form->input('Nội dung', ['type' => 'textarea', 'value' => $editPost->post_content, 'class' => 'ckeditor form-control', 'id' => 'post-content', 'name' => 'post_content'])?>
        </div>
    </div>
    <div class="col-md-4">
        <div class="form-group">
            <label>Ảnh đại diện</label>
            <?= $this->Form->input('post_image', ['class' => 'form-control', 'type' => 'hidden', 'value' => $editPost->post_image]);?>
